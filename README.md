...
## 📖 The Story Behind It (Proof of Grit)
...
3. **[TARGET] MCA-first filter (Proprietary Logic)**
...
flowchart TD
    Q[User Query] --> P1[Preprocessing: Classify, Keywords, Synonyms]
    P1 --> MCA[MCA-first Coverage Gate]
    P1 --> FAISS[FAISS (BGE-Large)]
    P1 --> BM25[BM25 Sparse]
    MCA --> U[Union]
    FAISS --> U
    BM25 --> U
    U --> CE[Cross-Encoder Reranking]
    CE --> LLM[gpt-4o-mini Answer]
...
All databases, indexes, and logs are included for immediate testing.
...
### Requirements
  ...
  - Ollama running locally with `qwen2.5:14b` (`ollama pull qwen2.5:14b`).
...
## 🚀 Ready for Production? (Partnerships & Investment)
